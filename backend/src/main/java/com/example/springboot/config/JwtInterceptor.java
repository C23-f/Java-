 ...
 *
 * 特殊规则：小区(/api/community/**)、设施(/api/facility/**)、街道(/api/district/**)
 * 的 GET 查询请求公开放行，供前端地图页面未登录时浏览；增删改(POST/PUT/DELETE)
 * 仍需登录+admin/operator角色。用户/角色/操作日志管理仅 admin。
 */
@Component
public class JwtInterceptor implements HandlerInterceptor {
    ...
    private static final List<String> DATA_WRITE_PREFIX =
            List.of("/api/community", "/api/facility", "/api/category", "/api/district");

    /** 公开只读路径前缀（GET 请求无需登录） */
    private static final List<String> PUBLIC_READ_PREFIX =
            List.of("/api/community/", "/api/facility/", "/api/district/");
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        // 小区/设施/街道的 GET 查询请求公开放行（前端地图页面无需登录即可浏览POI和街道）
